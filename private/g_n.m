function result = g_n(n,j,k_1,k_2,h,c,M,omega,x_1,x_2,x_3)
    A_nj = get_A(n,j,k_1,k_2,h,M,omega,x_1,x_2,x_3);
    B_n = get_B(n,j,k_1,k_2,h,M,omega,x_1,x_2,x_3);
    C_nj = get_C(n,j,k_1,k_2,h,M,omega,x_1,x_2,x_3);
    ...
    %     -1./(B_n-4*A_nj*h).*mysqrt(4*C_nj*h)./mysqrt((4*A_nj*h+4*C_nj*h-B_n)).*EE(1/2*(4*A_nj*h+4*C_nj*h-B_n)));
    % 
    % result_tmp(5,:,:,:,:) =  D_nj.*exp(-1i*B_n/4).*exp(-1i*A_nj*h)./A_nj./B_n .* ...
    %     (mysqrt(4*C_nj*h)./mysqrt((4*A_nj*h+4*C_nj*h+B_n)).*EE(1/2*(4*A_nj*h+4*C_nj*h+B_n))...
    %     -mysqrt(4*C_nj*h)./mysqrt((4*A_nj*h+4*C_nj*h-B_n)).*EE(1/2*(4*A_nj*h+4*C_nj*h-B_n)));
    
    result_tmp(4,:,:,:,:) = 4*D_nj*h.*exp(-1i*B_n/4).*exp(-1i*A_nj*h)./B_n .* ...
        (1./(B_n+4*A_nj*h).*mysqrt(4*C_nj*h)./mysqrt((4*A_nj*h+4*C_nj*h+B_n)).*EE(1/2*(4*A_nj*h+4*C_nj*h+B_n))...
        +1./(B_n-4*A_nj*h).*mysqrt(4*C_nj*h)./mysqrt((4*A_nj*h+4*C_nj*h-B_n)).*EE(1/2*(4*A_nj*h+4*C_nj*h-B_n)));
    
    result_tmp(4,ind_B0) = 2*D_nj(ind_B0).*exp(-1i*A_nj(ind_B0)*h)./A_nj(ind_B0)/h  .*(-mysqrt(4*C_nj(ind_B0)./(4*C_nj(ind_B0)+4*A_nj(ind_B0)).^3)/2.*EE((2*A_nj(ind_B0)+2*C_nj(ind_B0))*h)+mysqrt(C_nj(ind_B0)./(C_nj(ind_B0)+A_nj(ind_B0)))*h/2.*E_prime((2*A_nj(ind_B0)+2*C_nj(ind_B0))*h));
